...
// Récupération des ensembles en français depuis l'API
$response_sets_fr = file_get_contents($url_sets_fr);
...
if ($response_sets_fr === false) {
    ...
    $sets_data_fr = json_decode($response_sets_fr, true);

    // Regrouper les ensembles par bloc
    $blocks = array();
    foreach ($sets_data_fr['data'] as $set) {
        $block = $set['series'];
...
SELECT pc.*, ps.name AS set_name, pb.name AS block, pcs.card_secondary 
FROM pokemon_card pc 
JOIN pokemon_set ps ON pc.set_id = ps.id_set AND ps.name = 'Calendrier des Fêtes 2023' 
LEFT JOIN pokemon_block pb ON ps.block_id = pb.id_block 
LEFT JOIN (
    SELECT pcs.card_id, 
    CASE WHEN COUNT(pcs.card_id) = 0 THEN NULL 
    ELSE GROUP_CONCAT(pcs.name SEPARATOR ',') 
    END AS card_secondary 
    FROM pokemon_card_secondaire pcs 
    GROUP BY pcs.card_id
) pcs ON pc.id_card = pcs.card_id 
ORDER BY pc.number


ca devrais etre la bonne requette pour les card et carde secondair sof il manque 1 left join pour recupere si la card et recupere
...
SELECT 
    ...
    pb.name AS block, 
    GROUP_CONCAT(
        CONCAT(pcs.card_secondary, '=', pcs.card_secondary_owned) 
        ORDER BY pcs.card_secondary SEPARATOR ', '
    ) AS card_secondary
FROM 
    pokemon_card pc 
JOIN 
...
LEFT JOIN 
    (SELECT 
         ...
         IFNULL(MAX(CASE WHEN puc.user_id = 1 AND puc.user_card_actif = 1 THEN 1 ELSE 0 END), 0) AS card_secondary_owned
     FROM 
         pokemon_card pc 
     LEFT JOIN 
         pokemon_card_secondaire pcs ON pc.id_card = pcs.card_id 
     LEFT JOIN 
         pokemon_user_card puc ON pcs.id_pk_card_secondaire = puc.card_secondaire_id 
     GROUP BY 
         pc.id_card, pcs.name
...
SELECT 
    ...
    IF(puc.user_id IS NOT NULL, 1, 0) AS user_card
FROM 
    pokemon_card pc 
JOIN 
    pokemon_set ps ON pc.set_id = ps.id_set AND ps.name = 'Calendrier des Fêtes 2023' 
LEFT JOIN 
	pokemon_user_card puc ON puc.user_id = 1 AND pc.id_card = puc.card_id AND puc.user_card_actif = 1
LEFT JOIN 
    pokemon_block pb ON ps.block_id = pb.id_block 
LEFT JOIN 
    (SELECT 
...
FROM 
    pokemon_card pc 
JOIN 
    pokemon_set ps ON pc.set_id = ps.id_set AND ps.name = ?
LEFT JOIN 
	pokemon_user_card puc ON puc.user_id = 1 AND pc.id_card = puc.card_id AND puc.user_card_actif = 1
LEFT JOIN 
    pokemon_block pb ON ps.block_id = pb.id_block 
...
LEFT JOIN 
	pokemon_user_card puc ON puc.user_id = t.user_id AND pc.id_card = puc.card_id AND puc.user_card_actif = 1
LEFT JOIN 
    pokemon_block pb ON ps.block_id = pb.id_block 
LEFT JOIN 
    (SELECT 
         pc.id_card,
         pcs.name AS card_secondary,
         IFNULL(MAX(CASE WHEN puc.user_id = t.user_id AND puc.user_card_actif = 1 THEN 1 ELSE 0 END), 0) AS card_secondary_owned
     FROM 
         pokemon_card pc 
     LEFT JOIN 
	token t ON t.token = 'Vcs+bqCb=.ZLaWNkH@.85KbKUADe+VO@'
...
SELECT pc.*, ps.name, pb.name AS block, pr.name AS rarete, pr.image AS rarete_img, pe.name AS energie, pe.image AS energie_img, GROUP_CONCAT(CONCAT(pcs.card_secondary, '=', pcs.card_secondary_owned) ORDER BY pcs.card_secondary SEPARATOR ', ') AS card_secondary, IF(puc.user_id IS NOT NULL, 1, 0) AS user_card FROM pokemon_card pc JOIN pokemon_set ps ON pc.set_id = ps.id_set AND ps.name = "Calendrier des Fêtes 2023" LEFT JOIN token t ON t.token = 'Vcs+bqCb=.ZLaWNkH@.85KbKUADe+VO@' LEFT JOIN pokemon_user_card puc ON puc.user_id = t.user_id AND pc.id_card = puc.card_id AND puc.user_card_actif = 1 LEFT JOIN pokemon_rarete pr ON pr.id_rarete = pc.rarete_id LEFT JOIN pokemon_energie pe ON pe.id_energie = pc.energy_id
LEFT JOIN pokemon_block pb ON ps.block_id = pb.id_block LEFT JOIN (SELECT DISTINCT pc.id_card, pcs.name AS card_secondary, IFNULL(MAX(CASE WHEN puc.user_id = t.user_id AND puc.user_card_actif = 1 THEN 1 ELSE 0 END), 0) AS card_secondary_owned FROM pokemon_card pc LEFT JOIN token t ON t.token = 'Vcs+bqCb=.ZLaWNkH@.85KbKUADe+VO@' LEFT JOIN pokemon_card_secondaire pcs ON pc.id_card = pcs.card_id LEFT JOIN pokemon_user_card puc ON pcs.id_pk_card_secondaire = puc.card_secondaire_id GROUP BY pc.id_card, pcs.name) pcs ON pc.id_card = pcs.id_card GROUP BY pc.id_card ORDER BY pc.cardId
...
SELECT 
    ...
    IFNULL(GROUP_CONCAT(CONCAT(pcs.card_secondary, '=', IFNULL(pcs.card_secondary_owned, 0)) ORDER BY pcs.card_secondary SEPARATOR ', '), '') AS card_secondary, 
    IF(puc.user_id IS NOT NULL, 1, 0) AS user_card 
FROM 
    pokemon_card pc 
JOIN 
...
LEFT JOIN 
    ...
    LEFT JOIN 
        pokemon_user_card puc ON pcs.id_pk_card_secondaire = puc.card_secondaire_id 
    GROUP BY 
        pc.id_card, pcs.name) pcs ON pc.id_card = pcs.id_card 
GROUP BY 
    pc.id_card 
ORDER BY 
    pc.cardId;
...
pe.id_pk_etat, pe.etat, COALESCE(COUNT(puc.id_pk_user_card), 0) AS nombre_de_cartes

SELECT pe.id_pk_etat, pe.etat, COALESCE(COUNT(DISTINCT puc.id_pk_user_card), 0) AS nombre_de_cartes
FROM pokemon_etat pe
LEFT JOIN pokemon_user_card puc ON pe.id_pk_etat = puc.etat_id
INNER JOIN token t ON t.token = 'Vcs+bqCb=.ZLaWNkH@.85KbKUADe+VO@'
INNER JOIN pokemon_card_secondaire pcs ON puc.card_secondaire_id = pcs.id_pk_card_secondaire
INNER JOIN pokemon_card pc ON pcs.card_id = pc.id_card
INNER JOIN pokemon_set ps ON pc.set_id = ps.id_set AND ps.name = 'Calendrier des Fêtes 2023'
WHERE puc.user_card_actif = 1 
AND puc.user_id = t.user_id
AND pc.cardId = 1
GROUP BY pe.id_pk_etat, pe.etat; -- Grouper par l'état pour compter le nombre de cartes par état
